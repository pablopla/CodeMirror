...
(function(mod) {
  ...
  function CodeMirror(place, options) {
    ...
    if (options.direction && options.direction == "rtl")
  ...
  function directionChanged(cm) {
    if (cm.options.direction == "ltr") {
        rmClass(cm.display.wrapper, "CodeMirror-rtl");
    } else {
        addClass(cm.display.wrapper, "CodeMirror-rtl");
    }
    clearCaches(cm);
    var doc = cm.doc;
  ...
  function drawSelectionRange(cm, range, output) {
    ...
    function drawForLine(line, fromArg, toArg) {
      ...
      iterateBidiSections(getOrder(cm, lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function(from, to, dir) {
  ...
  function cursorCoords(cm, pos, context, lineObj, preparedMeasure) {
    ...
    var order = getOrder(cm, lineObj), ch = pos.ch;
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    var bidi = getOrder(cm, lineObj), dist = lineObj.text.length;
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(cm, lineObj, from, 1) : to - from <= 1) {
      ...
      if (bidi) {
        middle = from;
        for (var i = 0; i < step; ++i) middle = moveVisually(cm, lineObj, middle, 1);
      }
  ...
  CodeMirror.prototype = {
    ...
    setLineDirection: methodOp(function(handle, direction) {
      return changeLine(this, handle, "direction", function(line) {
        line.direction = direction;
        delete line.order;
        var otherDirection = direction == "rtl" ? "ltr" : "rtl";
        this.removeLineClass(line, "wrap", "CodeMirror-" + otherDirection);
        this.addLineClass(line, "wrap", "CodeMirror-" + direction);
        return true;
      });
    }),
    ...
    lineInfo: function(line) {
      ...
      return {line: n, handle: line, text: line.text, gutterMarkers: line.gutterMarkers,
              ...
              widgets: line.widgets, direction: line.direction || this.options.direction};
    },
  ...
  option("direction", "ltr", directionChanged, true);
  ...
  function buildLineContent(cm, lineView) {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      if (hasBadBidiRects(cm.display.measure) && (order = getOrder(cm, line)))
  ...
  function getOrder(cm, line) {
    var order = line.order;
    if (order == null) order = line.order = bidiOrdering(line.text, line.direction || cm.options.direction);
    return order;
  }
  ...
  function lineLeft(cm, line) { var order = getOrder(cm, line); return order ? bidiLeft(order[0]) : 0; }
  function lineRight(cm, line) {
    var order = getOrder(cm, line);
  ...
  function lineStart(cm, lineN) {
    ...
    var order = getOrder(cm, visual);
    var ch = !order ? 0 : order[0].level % 2 ? lineRight(cm, visual) : lineLeft(cm, visual);
    return Pos(lineN, ch);
  }
  function lineEnd(cm, lineN) {
    ...
    var order = getOrder(cm, line);
    var ch = !order ? line.text.length : order[0].level % 2 ? lineLeft(cm, line) : lineRight(cm, line);
  ...
  function moveVisually(cm, line, start, dir, byUnit) {
    var bidi = getOrder(cm, line);
  ...
  var bidiOrdering = (function() {
    ...
    var isNeutral = /[stwN]/, isStrong = /[LRr]/, countsAsLeft = /[Lb1n]/, countsAsRight = /[Lb1n]/, countsAsNum = /[1n]/;
    ...
    return function(str, direction) {
      var bidiRE = (direction == "ltr" ? /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/ : /[^\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/);
      var outerType = direction == "ltr" ? "L" : "R";
      var countsAsDirection = direction == "ltr" ? countsAsLeft : countsAsRight;
      ...
      var order = [], m;
      for (var i = 0; i < len;) {
        if (countsAsDirection.test(types[i])) {
          var start = i;
          for (++i; i < len && countsAsDirection.test(types[i]); ++i) {}
          order.push(new BidiSpan(0, start, i));
        } else {
          var pos = i, at = order.length;
          for (++i; i < len && types[i] != outerType; ++i) {}
          for (var j = pos; j < i;) {
